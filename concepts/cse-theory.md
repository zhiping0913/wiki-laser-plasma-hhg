...
Pirozhkov et al. (2006) developed the sliding mirror model for attosecond pulse generation in the relativistic regime. In the dense plasma limit, perpendicular electron motion is suppressed by the charge-separation field, and electrons move only along the target surface.
...
1. **Phase I**: Restoring Coulomb force (E_x) accelerates electron away from surface
2. **Phase II**: Laser E_y field opposes B_z-driven acceleration; p_y passes through zero → **REFLECTED emission** (v_x/c ~ -0.96)
3. **Phase III**: Electron passes through E_y node/B_z antinode; forward acceleration
4. **Phase IV**: p_y approaches zero → **FORWARD emission** (v_x/c ~ +0.98); electron enters plasma

### Key Results
- Reflected emission ~30× stronger than forward (bunch density ~250n_c vs ~50n_c)
- Pulse duration ~27 as (FWHM) for ω/ω_L = 100-200 filtering
- Figure-eight trajectories only in relativistically TRANSPARENT plasmas (thin ~30nm targets)
...
Liu et al. (2024) demonstrated EUV and x-ray generation from a propagating nanometer electron layer during few-cycle laser interaction with solid targets.
